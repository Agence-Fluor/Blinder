...
<style>
  ...
  :global(.custom-slider-thumb::-webkit-slider-thumb) {
    ...
    margin-top: -10px; /* Center for standard sliders */
  }
  
  :global(.custom-slider-thumb::-moz-range-thumb) {
    width: 24px;
    height: 24px;
    border-radius: 50%;
    background-color: white;
  ...
  /* Specific reset for Double Slider thumb margin if needed */
  :global(.relative .custom-slider-thumb::-webkit-slider-thumb) {
    margin-top: 0; 
  }
</style>
  ...
  {:else}
    ...
    {#if activeChatId && activeSession}
      <div class="fixed inset-0 bg-background z-30 flex flex-col">
        <!-- Chat Header -->
        <div class="h-16 bg-surface flex items-center px-4 border-b border-white/5 shadow-md">
          <button onclick={() => activeChatId = null} class="mr-4 text-gray-400 p-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width={2} d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <div class="flex items-center">
            <img src={activeSession.match.avatarUrl} alt="Avatar" class="w-10 h-10 rounded-full border border-primary bg-surface2" />
            <div class="ml-3">
              <div class="font-bold text-white">
                <span class="text-primary">{activeSession.match.searchProfileName}</span> {activeSession.match.funWord}
              </div>
              <div class="text-xs text-gray-400 flex flex-col">
                <span>{activeSession.match.age} ans • {activeSession.match.profession || 'Mystère'}</span>
                <span>{activeSession.match.religion} • {activeSession.match.bodyType}</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <div 
                class={`max-w-[75%] px-4 py-2 rounded-2xl text-sm ${
                  msg.senderId === 'user'
                    ...
                    : 'bg-surface2 text-gray-200 rounded-bl-none'
                }`}
              >
                {msg.text}
              </div>
        ...
        <!-- Chat Input -->
        <div class="p-4 bg-surface border-t border-white/5 pb-safe">
          <form 
            onsubmit={(e) => {
			  e.preventDefault()
            ...
            class="flex items-center bg-background rounded-full px-4 py-2 border border-gray-700 focus-within:border-primary transition-colors"
          >
            <input 
              name="msg" 
              type="text" 
            ...
            />
            <button type="submit" class="ml-2 text-primary">
    ...
    {:else}
        ...
        {#if activeTab === 'chat'}
          <div class="space-y-4">
            {#if sessions.length === 0}
              <div class="flex flex-col items-center justify-center h-[60vh] text-gray-500 text-center animate-pulse">
                <div class="mb-4">Recherche de compatibilités...</div>
                <div class="text-sm">Vos profils de recherche travaillent pour vous.</div>
              </div>
            {:else}
              {#each sessions as session (session.matchId)}
                <div 
                  onclick={() => activeChatId = session.matchId}
                  onkeydown={() => activeChatId = session.matchId}
                  role="button"
                  tabindex="0"
                  class="flex items-center p-4 bg-surface rounded-2xl shadow-lg active:scale-[0.98] transition-transform duration-200 cursor-pointer border border-white/5"
                >
                  <div class="relative">
                    <img 
                      src={session.match.avatarUrl} 
                      alt="Avatar" 
                      class="w-14 h-14 rounded-full object-cover border-2 border-primary bg-surface2"
                    />
                    {#if session.unread}
                      <div class="absolute top-0 right-0 w-3 h-3 bg-primary rounded-full border border-surface"></div>
                    {/if}
                  </div>
                  <div class="ml-4 flex-1">
                    <div class="flex justify-between items-baseline">
                      <h3 class="text-white font-semibold text-lg">
                        <span class="text-primary">{session.match.searchProfileName}</span> 
                        <span class="text-gray-300 ml-1 font-light italic">{session.match.funWord}</span>
                      </h3>
              ...
              <div class="bg-surface p-5 rounded-2xl shadow-lg border border-white/5">
                ...
                {#if isEditingUser}
                  <div class="space-y-4 animate-in fade-in duration-300">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      <div>
                        <label class="text-xs text-gray-500 uppercase font-bold">Taille (cm)</label>
                        <input type="number" bind:value={userProfile.height} class="w-full bg-background p-2 rounded text-white border border-gray-700 focus:border-primary outline-none mt-1" />
                      </div>
                      <div>
                    ...
                    <!-- Interests -->
                    <h4 class="text-primary font-bold text-xs uppercase tracking-wider mt-4 mb-2 border-b border-white/10 pb-1">Intérêts</h4>
                    <div>
                      <div class="flex flex-wrap gap-2">
                        {#each INTERESTS_LIST as interest}
                          {@const selected = userProfile.interests.includes(interest)}
                          <button
                            onclick={() => {
                              if (selected) {
                            ...
                            class={`px-3 py-1 rounded-full text-xs border ${selected ? 'bg-primary/20 border-primary text-primary' : 'border-gray-700 text-gray-500'}`}
                          >
                            {interest}
                          </button>
                        {/each}
                  ...
                  <div class="text-center space-y-2">
                    ...
                    <div class="flex flex-wrap justify-center gap-2 mt-4">
                      {#each userProfile.interests as i}
                        <span class="px-2 py-1 bg-surface2 text-xs rounded text-gray-300">{i}</span>
                      {/each}
                    </div>
                  </div>
                {/if}
                        ...
                        <div class="space-y-6">
                          ...
                          <div>
                            <div class="flex justify-between mb-2">
                              <label class="text-[10px] text-gray-400 uppercase font-bold">Age</label>
                            </div>
                            <RangeSlider 
							  label="age"
             				  min={18} max={99} 
      ...
      <div class="fixed bottom-0 left-0 right-0 h-16 border-t border-white/5 flex justify-around items-center z-20 pb-safe  backdrop-blur-md">
        <button 
          ...
          class={`flex flex-col items-center justify-center w-full h-full ${activeTab === 'chat' ? 'text-primary' : 'text-gray-500'}`}
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button 
          ...
          class={`flex flex-col items-center justify-center w-full h-full ${activeTab === 'profile' ? 'text-primary' : 'text-gray-500'}`}
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
          </svg>
